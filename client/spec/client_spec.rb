...
describe "Caldecott Client" do
  ...
  it "should raise an error if tunnel url is not provided" do
    expect { Caldecott::Client.start({}) }.to raise_error(Caldecott::InvalidTunnelUrl)
  end

  it "should sanitize URL" do
    tun_url = "cloudfoundry.COM "
    sanitized_url = "https://cloudfoundry.com"
    options = { :tun_url => tun_url }
    client = Caldecott::Client::CaldecottClient.new(options)
    client.sanitize_url(tun_url).should == sanitized_url
  end

  it "should send buffered data from tunnel to local server" do
  ...
  it "should send data from local server to tunnel" do
    options = { :tun_url => "http://tunnel.cloudfoundry.com" }
    client = Caldecott::Client::CaldecottClient.new(options)
